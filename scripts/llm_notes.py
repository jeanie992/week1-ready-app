...
def _build_prompt(statements, years, company, extra_context):
    ...
    return (
        ...
        f"{extra_context}\n\n{table}\n\n"
        "의미있는 핵심 계정(20~30개)에 대해 추세·비중·이상치 근거로 한 줄 인사이트를 쓴다.\n"
        "규칙 : 한국어 : 문장 끝 온점 없음 : 하이픈 대신 콜론( : ) : 각 노트 40자 이내 : 출처 없는 추정 금지\n"
        'JSON 배열만 출력(다른 말 금지) : [{"statement":"IS|BS|CF","account":"계정명","note":"..."}]'
    )
